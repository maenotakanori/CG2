...
extern IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND nWnd, UINT msg, WPARAM wParam, LPARAM lParam);
...
Matrix4x4 MakePerspectiveFovMatrix(float fovY, float aspectRatio, float nearClip, float farClip) {
	Matrix4x4 result = {};
	float a = fovY / 2;
	float aCot = 1 / std::tan(a) / aspectRatio;
	float bCot = 1 / std::tan(a);
	float c = farClip / (farClip - nearClip);
	float d = -nearClip * farClip / (farClip - nearClip);
...
Matrix4x4 MakeOrthographicMatrix(float left, float top, float right, float bottom, float nearClip, float farClip) {
	...
	float c = 1 / ((farClip - nearClip));
	float d = (left + right) / (left - right);
	float e = (top + bottom) / (bottom - top);
	float f = nearClip / (nearClip - farClip);
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	D3D12_COMMAND_QUEUE_DESC commandQueuDesc{};
	hr = device->CreateCommandQueue(&commandQueuDesc,
	...
	*wvpData = MakeIdentity4x4();
	...
	ID3D12Resource* transformResource = CreateBufferResource(device, sizeof(Matrix4x4));
	...
	*transformationMatrixDataSprite = MakeIdentity4x4();
	...
	materialSpriteResource->Map(0, nullptr, reinterpret_cast<void**>(&materialDataSprite));
	materialDataSprite->color = Vector4(1.0f, 0.0f, 0.0f, 1.0f);
	...
	ID3D12Resource* depthStecilResource = CreateDepthStencilTextureResource(device, kClientWidth, kClientHeight);
	ID3D12DescriptorHeap* dsvDescriptorHeap = CreateDescriptorHeap(device, D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 1, false);
	D3D12_DEPTH_STENCIL_VIEW_DESC dsvDesc{};
	dsvDesc.Format = DXGI_FORMAT_D24_UNORM_S8_UINT;
	dsvDesc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
	device->CreateDepthStencilView(depthStecilResource, &dsvDesc, dsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart());
	...
	*transformationMatrixDataSprite = worldViewProjectionMatrixSprite;
	...
	while (msg.message != WM_QUIT) {
		...
		if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
			...
			ImGui::Begin("Window");
			//↓ImGuiここから
			ImGui::DragFloat3("color",&materialDataSprite->color.x,0.01f);
